...
impl ServerState {
    ...
    /// filtered by the same `Glob::new` validity rule the matcher globs
    /// use: one invalid configured name must not poison the whole
    /// registration (a strict client could reject it wholesale).
    /// Sorted, deduplicated.
