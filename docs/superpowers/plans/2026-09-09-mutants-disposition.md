...
- **The table governs**: no implementation outside a table row's disposition; a row may be revised during implementation only with the reason recorded in the table file itself (append a `revised:` note), never silently.
