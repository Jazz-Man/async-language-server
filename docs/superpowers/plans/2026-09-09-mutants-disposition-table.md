...
| `find_descendant` @ tree_sitter_utils.rs:122 | `-> Option<Node> with None` | b | "search descendants in a depth-first manner" | same test: deepest-first first match asserted |
...
| `Document::node_at_position` @ document.rs:231 | `-> Option<Node> with None` | b | "Returns a Node at the given LSP position, if one exists" — the hover/completion primitive for downstream servers | same test: valid position → `Some`; out-of-tree → `None` |
| `Document::node_at_position_named` @ document.rs:239 | `-> Option<Node> with None` | b | Documented named-only variant | same test: named node at an anonymous-token position |
...
| `absolute_position` @ conversion.rs:968 | `-> Position with Default::default()` | b | Delta-fold seed for semanticTokens/full/delta: the inserted stream re-anchors at the cached prefix's end | W0 `absolute_position_folds_deltas`: `[(1,0),(0,5)]` → `{1, 5}`; empty → origin |
