...
**Goal:** Bring the two-member workspace to convention parity and close the visibility looseness: a clean `lsp_macros` skeleton replacing the invalid placeholder, `[workspace.lints]`/`[workspace.dependencies]` inheritance, manifest parity for `macros/Cargo.toml`, English-only manifest comments, and the 49-item `pub` → `pub(crate)` sweep confined to `src/lsp_requests/`.
...
| `src/lsp_requests/mod.rs` | `pub trait Request` → `pub(crate)`; stamper's `pub struct $req;` → `pub(crate) struct $req;` | modify 2 lines |
| `src/lsp_requests/<17 files>` | hand-written request structs → `pub(crate)` | modify 1 line each |
...
- Modify: 7 more files carrying `#[cfg(feature = "tracing")]` lines: `src/server/with_state/mod.rs` (10 sites), `src/server/state/documents.rs` (3), `src/server/with_state/initialize.rs` (2), `src/workspace/diagnostics.rs` (2), `src/documents/matcher.rs` (2), `src/lsp_requests/conversion.rs` (1), `src/workspace/walker.rs` (1)
...
src/lsp_requests/conversion.rs:950
...
### Task 4: The visibility sweep — 49 `pub` → `pub(crate)` in `src/lsp_requests/`

**Files:**
- Modify: `src/lsp_requests/mod.rs` (two lines: the trait declaration and the stamper's struct emission)
...
- [ ] **Step 3: Verify nothing `pub` remains in `src/lsp_requests/`**

Run: `grep -rnE "^pub (trait|struct|enum|fn|mod|use|const|type)" src/lsp_requests/`
...
Changed files: `src/lsp_requests/mod.rs` plus the 17 files above. This is the plan's final checkpoint — the battery in Step 4 doubles as the plan-level done criterion.
