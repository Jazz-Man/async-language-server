...
- Battery after every task: `cargo build --workspace --all-targets && cargo test --workspace && cargo fmt --check && cargo clippy --workspace --all-targets -- -D warnings`; the three-configuration battery (`--no-default-features`, `--all-features`) runs in Tasks 2, 7, and 9.
