...
3. Author `dupes.toml` encoding only invariants that hold TODAY (the arch-lint philosophy): **NO `exclude_tests`** (owner override 2026-08-31: "тести це теж код" — the test-side duplication this cycle kept removing is exactly what the detector must keep watching; deliberate test parallelism gets reasoned ignore entries, not a blanket knob), `min_nodes = 15` (skip trivial fns); tune per the baseline report. Intentional duplicates get `cargo dupes ignore <fingerprint> --reason "..."` entries (stored in `.dupes-ignore.toml`, committed) — never threshold-loosening without a reason.
